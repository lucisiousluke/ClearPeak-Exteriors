...
// Writes plain CSS (not a .ts data module) since these values need to reach
// Tailwind's generated CSS as custom properties, not JS-imported strings.
async function fetchBrandColorsCss() {
  ...
  return (
    ...
    ` * Do not edit directly — edit content in the Sanity Studio (Brand Colors) instead, then\n` +
    ` * re-run \`npm run sync:sanity\` (or just \`npm run develop\` / \`npm run build\`). */\n\n` +
    `:root {\n` +
    `  --color-aqua: ${primary};\n` +
...
async function main() {
  ...
  writeFileSync(path.join(stylesDir, "brandColors.generated.css"), await fetchBrandColorsCss());
  console.log(`✓ wrote src/styles/brandColors.generated.css`);
